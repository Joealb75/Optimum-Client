...
export const OfficeDashboard = () => {
  ...
  const sortedConsultations = filteredConsultations.sort((consult1, consult2) => new Date(consult1.created_date) - new Date(consult2.created_date)); 
  ...
  return (
    <>
      ...
      <div className="p-4">
        {currentUser ? (
          <p className="text-xl font-semibold">Welcome, {currentUser.first_name} {officeUser.phone_number}</p>
        ) : (
      ...
      <section className="p-4">
        <div className="flex justify-between items-center mb-4">
          ...
          <div className="flex items-center space-x-4">
            <button className="bg-gray-800 text-white px-4 py-2 rounded hover:bg-[#228B22]">
              View All
            </button>
            <select 
              value={statusFilter} 
            ...
            sortedConsultations.map((consultation) => {
              ...
              return (
                <div key={consultation.id} className="min-w-[300px]">
                  <ConsultationCard consultation={consultation} assignedUserName={assignedUserName} />
                </div>
              );
            })
          ) : (
